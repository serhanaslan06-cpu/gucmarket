...
export default function Products(){
 ...
 const [allProducts,setAllProducts]=useState<any[]>(products);\n const [dbCategories,setDbCategories]=useState<any[]>([]);\n useEffect(()=>{fetch('/api/categories',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then(data=>{if(Array.isArray(data)&&data.length){setDbCategories(data);}}).catch(()=>{}); fetch('/api/products',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then(data=>{if(Array.isArray(data)&&data.length){setAllProducts(data.map((p:any)=>({id:p.id,name:p.name,brand:p.brand?.name||'',cat:p.category?.name||'',spec:p.description||'',price:p.price?`${p.price} ${p.currency||'TRY'}`:'Fiyat için teklif al',city:p.city||'',seller:p.supplier?.companyName||'',technical:Object.fromEntries((p.technical||[]).map((v:any)=>[v.criterion?.key,v.value]))})));}}).catch(()=>{});},[]);
 ...
 const renderCriterion=(c:Criterion)=>{
   ...
   if(c.dependsOn&&!criteriaFilters[c.dependsOn]?.values?.[0]) return <div key={c.key} style={{marginBottom:14,opacity:.6}}><p style={{marginBottom:6,fontWeight:700}}>{c.label}</p><div style={{padding:8,border:'1px solid #ddd',borderRadius:6}}>Önce {activeCriteria.find((x:any)=>x.key===c.dependsOn)?.label || categoryCriteria[cat]?.find(x=>x.key===c.dependsOn)?.label} seçin</div></div>;
 ...
 return <main className="container" style={{paddingTop:35}}><h1>Ürün Kataloğu</h1><p style={{color:'var(--muted)'}}>Kategori seçildiğinde admin tarafından tanımlanan kriterler, satıcı ürün değerleri üzerinden filtrelenir.</p><div style={{marginTop:20}}><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Ürün, marka, model veya parça kodu ara..." style={{width:'100%',padding:14,border:'1px solid #d8deea',borderRadius:10}}/></div><div style={{display:'grid',gridTemplateColumns:'300px 1fr',gap:24,marginTop:25}}><aside className="card" style={{padding:20}}><b>Filtreler</b><hr/><p>Kategori</p><select value={cat} onChange={e=>{setCat(e.target.value);setCriteriaFilters({})}} style={{width:'100%',padding:10}}><option>Tüm kategoriler</option>{categoryNames.map((x:string)=><option key={x}>{x}</option>)}</select><p>Marka</p><select value={brand} onChange={e=>setBrand(e.target.value)} style={{width:'100%',padding:10}}><option>Tüm markalar</option>{brands.map(x=><option key={x}>{x}</option>)}</select>{cat!=='Tüm kategoriler'&&<><hr/><p style={{fontWeight:800}}>{cat} Teknik Filtreleri ({activeCriteria.length})</p>{activeCriteria.map(renderCriterion)}</>}<p style={{marginTop:18,color:'var(--muted)',fontSize:13}}>Gösterilen ürün: {filtered.length}</p></aside><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>{filtered.map(p=><ProductCard key={p.id} p={p}/>)}</div></div></main>
}

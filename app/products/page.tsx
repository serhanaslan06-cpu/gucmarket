...
export default function Products(){
 ...
 const renderCriterion=(c:Criterion)=>{
   const filter=criteriaFilters[c.key]||{};
   if(c.type==='number') return <div key={c.key} style={{marginBottom:14}}>
     <p style={{marginBottom:6,fontWeight:700}}>{c.label}{c.unit?' ('+c.unit+')':''}</p>
     <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
       <input type="number" value={filter.min||''} onChange={e=>setFilter(c.key,{...filter,min:e.target.value})} placeholder="Min" style={{width:'100%',padding:8}}/>
       <input type="number" value={filter.max||''} onChange={e=>setFilter(c.key,{...filter,max:e.target.value})} placeholder="Max" style={{width:'100%',padding:8}}/>
     </div>
   </div>;

   if(c.type==='multiselect') return <div key={c.key} style={{marginBottom:14}}>
     <p style={{marginBottom:6,fontWeight:700}}>{c.label}</p>
     <div style={{display:'grid',gap:5}}>{(c.values||[]).map(v=><label key={v} style={{fontSize:13,display:'flex',gap:7,alignItems:'center'}}><input type="checkbox" checked={(filter.values||[]).includes(v)} onChange={()=>toggleMulti(c.key,v)}/>{v}</label>)}</div>
   </div>;

   return <div key={c.key} style={{marginBottom:14}}>
     <p style={{marginBottom:6,fontWeight:700}}>{c.label}</p>
     <select value={filter.values?.[0]||''} onChange={e=>setFilter(c.key,{values:e.target.value?[e.target.value]:[]})} style={{width:'100%',padding:8}}>
       <option value="">Tümü</option>{(c.values||[]).map(v=><option key={v}>{v}</option>)}
     </select>
   </div>;
 };
 ...
 return <main className="container" style={{paddingTop:35}}>
   <h1>Ürün Kataloğu</h1>
   <p style={{color:'var(--muted)'}}>Kategori seçtiğinizde o kategoriye ait tüm teknik özellikler filtre kriteri olarak açılır.</p>
   ...
   <div style={{display:'grid',gridTemplateColumns:'300px 1fr',gap:24,marginTop:25}}>
     <aside className="card" style={{padding:20}}>
       <b>Filtreler</b><hr/>
       <p>Kategori</p>
       <select value={cat} onChange={e=>{setCat(e.target.value);setCriteriaFilters({})}} style={{width:'100%',padding:10}}>
         <option>Tüm kategoriler</option>{categories.map(x=><option key={x.name}>{x.name}</option>)}
       </select>
       <p>Marka</p>
       <select value={brand} onChange={e=>setBrand(e.target.value)} style={{width:'100%',padding:10}}>
         <option>Tüm markalar</option>{brands.map(x=><option key={x}>{x}</option>)}
       </select>
       {cat!=='Tüm kategoriler'&&<><hr/><p style={{fontWeight:800}}>{cat} Teknik Filtreleri ({activeCriteria.length})</p>{activeCriteria.map(renderCriterion)}</>}
       <p style={{marginTop:18,color:'var(--muted)',fontSize:13}}>Gösterilen ürün: {filtered.length}</p>
     </aside>

export type Criterion={key:string;label:string;type:'select'|'number'|'boolean'|'multiselect';unit?:string;values?:string[];filterable?:boolean};
...
export const categoryCriteria:Record<string,Criterion[]>={
 'UPS / KGK':[
 {key:'upsType',label:'UPS Tipi',type:'select',values:['Statik','Dinamik']},{key:'topology',label:'UPS Topolojisi',type:'select',values:['Online','Line-interactive','Offline']},{key:'powerKva',label:'Güç',type:'number',unit:'kVA'},{key:'activePowerKw',label:'Aktif Güç',type:'number',unit:'kW'},{key:'inputVoltage',label:'Giriş Gerilimi',type:'number',unit:'V'},{key:'outputVoltage',label:'Çıkış Gerilimi',type:'number',unit:'V'},{key:'inputPhase',label:'Giriş Fazı',type:'select',values:['1F','3F']},{key:'outputPhase',label:'Çıkış Fazı',type:'select',values:['1F','3F']},{key:'frequency',label:'Frekans',type:'number',unit:'Hz'},{key:'thdi',label:'THDi',type:'number',unit:'%'},{key:'thdv',label:'THDv',type:'number',unit:'%'},{key:'efficiency',label:'Verim',type:'number',unit:'%'},{key:'outputVoltageAdjustment',label:'Çıkış Voltajı Ayarı',type:'select',values:['Kullanıcı','Teknik Servis']},{key:'outputFrequencyAdjustment',label:'Çıkış Frekans Ayarı',type:'select',values:['Kullanıcı','Teknik Servis']},{key:'physicalStructure',label:'Fiziksel Yapı',type:'select',values:['Tower Kasa','Rack Tipi']},{key:'wheels',label:'Tekerlek',type:'boolean',values:['Var','Yok']},{key:'wallFlushMountable',label:'Duvara Sıfır Montajlanabilir',type:'boolean',values:['Evet','Hayır']},{key:'batteryVoltage',label:'Akü Gerilimi',type:'number',unit:'VDC'},{key:'batteryRuntime',label:'Akü Süresi',type:'number',unit:'dk'},{key:'externalBatteryCabinet',label:'Harici Akü Kabini Bağlantısı',type:'boolean',values:['Var','Yok']},{key:'maxBatteryAh',label:'Maksimum Bağlanabilecek Akü Kapasitesi',type:'number',unit:'Ah'},{key:'internalSnmp',label:'Dahili SNMP',type:'boolean',values:['Var','Yok']},{key:'externalSnmp',label:'Harici SNMP',type:'boolean',values:['Var','Yok']},{key:'batteryTemperatureMonitoring',label:'Akü Sıcaklık İzleme',type:'boolean',values:['Var','Yok']},{key:'dryContact',label:'Kuru Kontak Bağlantısı',type:'boolean',values:['Var','Yok']},{key:'batteryType',label:'Akü Tipi',type:'multiselect',values:['VRLA','Li-ion']},{key:'displayType',label:'Ekran Tipi',type:'multiselect',values:['LCD','LED','Dokunmatik LCD','Grafik LCD','Ekran Yok','Diğer']},{key:'parallelOperation',label:'Paralel Çalışma',type:'boolean',values:['Var','Yok']},{key:'modular',label:'Modüler',type:'boolean',values:['Var','Yok']}
 ],
 'Aküler':[
